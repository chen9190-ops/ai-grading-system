...
export default function GradingHistoryDetailPage() {
  ...
  return <MobileShell><MobileTopBar title="历史批改详情" />{!loaded ? <div className="mx-4 mt-5 h-40 animate-pulse rounded-2xl bg-white/60" /> : error || !detail ? <div className="mx-4 mt-5 rounded-2xl border border-red-200 bg-red-50 p-5 text-sm text-red-700">{error || "历史记录加载失败"}</div> : <div className="space-y-4 px-4 py-5"><section className="rounded-[22px] border border-white/80 bg-white/82 p-4 shadow-[0_7px_20px_rgba(30,41,59,.07)]"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h1 className="truncate text-base font-bold">{detail.title}</h1><p className="mt-1 text-xs text-slate-500">{detail.courseName} · {formatDate(detail.createdAt)}</p></div><strong className="shrink-0 text-lg text-blue-600">{formatScoreWithMaximum(detail.score)}</strong></div><div className="mt-3 border-t border-slate-100 pt-3 text-[10px] leading-5 text-slate-400"><p>requestId: {detail.requestId || "未记录"}</p><p>workflowRunId: {detail.workflowRunId || "未记录"}</p></div></section>{detail.markdown ? <section className="rounded-[22px] border border-white/80 bg-white/85 p-4 shadow-[0_7px_20px_rgba(30,41,59,.07)]"><GradingMarkdown content={detail.markdown} /></section> : <section className="rounded-[22px] border border-amber-200 bg-amber-50 p-6 text-center"><FileWarning className="mx-auto size-8 text-amber-500" /><p className="mt-3 text-sm font-semibold text-amber-800">该历史记录未保存完整的 AI 批改报告，无法查看详细解析。</p></section>}</div>}</MobileShell>;
}
