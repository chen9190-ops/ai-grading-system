...
export default async function TeacherAnalysisPage() {
  ...
  return (
    <>
      ...
      <TeachingReportGenerator studentCount={analysis.studentCount} averageScore={analysis.averageScore} weakPoints={analysis.knowledgePoints.map(({ name, count }) => ({ name, count }))} errorTypes={analysis.errorTypes.map(({ name, count }) => ({ name, count }))} />
    </>
  );
}
