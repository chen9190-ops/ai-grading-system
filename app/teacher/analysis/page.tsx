...
function buildAnalysis(records: RecordItem[]) {
  ...
  return {
    ...
    scoreDistribution: [
      { label: "0-5.9", count: scores.filter((score) => score < 6).length },
      { label: "6-6.9", count: scores.filter((score) => score >= 6 && score < 7).length },
      { label: "7-7.9", count: scores.filter((score) => score >= 7 && score < 8).length },
      { label: "8-8.9", count: scores.filter((score) => score >= 8 && score < 9).length },
      { label: "9-10", count: scores.filter((score) => score >= 9).length },
    ],
